...
int	parse_args(int ac, char **av, t_rules *rules)
{
	...
	if (parse_pos_int(av[1], &rules->num_philos))  return (1);
	if (parse_pos_int(av[2], &rules->time_to_die))  return (1);
	if (parse_pos_int(av[3], &rules->time_to_eat))  return (1);
	if (parse_pos_int(av[4], &rules->time_to_sleep))return (1);
