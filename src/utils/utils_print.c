...
/*   Updated: 2025/09/20 18:12:48 by nbariol-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	print_status(t_philo *ph, char *msg)
{
	...
	if (strcmp(msg, "died") == 0)
	{
		...
		printf("%lld %d %s\n", current_time_ms() - ph->rules->start_time,
			ph->id, msg);
	}
...
int	error_msg(const char *msg)
{
	...
	fprintf(stderr,
		"— check: " YELLOW
		"./philo <number_of_philosophers> <time_to_die_ms> ");
	fprintf(stderr,
		"<time_to_eat_ms> <time_to_sleep_ms> [must_eat_count]" RESET "\n");
	return (1);
}
